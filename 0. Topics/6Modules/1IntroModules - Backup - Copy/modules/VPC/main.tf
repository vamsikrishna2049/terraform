...
resource "aws_subnet" "pub_sn" {
  ...
  cidr_block              = "10.0.${count.index * 2}.0/24" #By default, it will create the subnet CIDR range
...
resource "aws_subnet" "pvt_sn" {
  ...
  vpc_id                  = aws_vpc.main.id
  cidr_block              = var.pvt_subnet_cidr_block[count.index] #We're passing the subnet CIDR range
