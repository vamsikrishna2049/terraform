...
#   subnet_id                   = element(aws_subnet.nvir_pub_sub.*.id, count.index)
#   vpc_security_group_ids      = ["${aws_security_group.Nvirg_sg.id}"]
...
# #Private Subnet
# resource "aws_instance" "Nvirg-private-servers" {
#   provider                    = aws.Nvirg
